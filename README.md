...
1. Представить проект по мониторингу/логированию/оповещению, на котором отобразить ключевые метрики, данный для логов и диаграмму с архитектурой решения
2. С помощью Docker Compose развернуть Prometheus, Grafana и ELK-стек.
3. Реализовать отправку метрик Spring Batch-приложением через Prometheus Agent или JMX Exporter.
4. Добавить логировние Spring Batch-операций. Настроить отправку логов в Elasticsearch через filebeat.
5. Создать дашборды сбора метрик приложения и работающие оповещения через Prometheus и(или) Grafana, выбрав наиболее подходящее решение и обосновав его.
6. Организовать централизованное логирование с ELK, продемонстрировав отображение собранных логов в Kibana.
...
- Созданы Grafana дашборды для отображения метрик
- Сконфигурировнны оповещения по 1-2 метрикам в Grafana/Prometheus
...
Вызов API должен сопровождаться логированием с трейсингом запросоов. У контекста запросу должно быть traceId, spanId, URI.
